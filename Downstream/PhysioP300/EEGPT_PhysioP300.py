...
from utils import *
from Modules.EEGPT.EEGPT_mcae import *
...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self, load_path="Modules/EEGPT/eegpt_mcae_58chs_4s_large4E.ckpt"):
        super().__init__()
        ...
        self.chan_scale = torch.nn.Parameter(
            torch.ones(1, self.chans_num, 1) + 0.001 * torch.rand((1, self.chans_num, 1)), requires_grad=True)

        self.target_encoder.load_state_dict(target_encoder_stat)

        self.linear_probe1 = LinearWithConstraint(self.embed_num * self.embed_dim, self.linear1_out_dim, max_norm=1)
        self.linear_probe3 = LinearWithConstraint(self.N * self.linear1_out_dim, 2, max_norm=0.25)
    ...
    def forward(self, x):
        ...
        z = self.target_encoder(x, self.chans_id.to(x))     # [B, N, embed_num, embed_dim]

        h = z.flatten(2)        # [B, N, embed_num * embed_dim]

        h = self.linear_probe1(self.drop(h))    #  [B, N, self.linear1_out_dim]

        h = h.flatten(1)            #  [B, N * self.linear1_out_dim]

        # h = self.linear_probe2(self.drop(h))    # [B, N * self.linear1_out_dim]

        h = self.linear_probe3(h)               #  [B, 2]
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(
            ...
            list(self.linear_probe3.parameters()),
            weight_decay=0.01)  #
...
if __name__ == "__main__":
    for seed in [7,718,1029]:
        ...
        for i, sub in enumerate(all_subjects):
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, num_workers=4, shuffle=True,persistent_workers=True)
            ...
            trainer = pl.Trainer(accelerator='cuda',
                         ...
                         logger=[pl_loggers.TensorBoardLogger(f'./logs/EEGPT/Seed{seed}', name="EEGPT_PhysioP300_tb",
                                                              version=f"subject{sub}"),
                                 pl_loggers.CSVLogger(f'./logs/EEGPT/Seed{seed}', name="EEGPT_PhysioP300_csv")])
