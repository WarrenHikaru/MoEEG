...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self):
        ...
        self.linear_probe2   =   LinearWithConstraint(468, 2, max_norm=0.25)
    ...
    def forward(self, x):
        ...
        x = x.float()
        z = self.feature(x, input_chans=[i for i in range(len(channels_index)+1)], return_all_tokens=True)
    ...
    for seed in [7,718,1029]:
        ...
        for i, sub in enumerate(all_subjects):
            ...
            trainer = pl.Trainer(accelerator='cuda',
                         ...
                         logger=[pl_loggers.TensorBoardLogger(f'./logs/LaBraM/Seed{seed}', name="MoEEG_PhysioP300_tb",
                                                              version=f"subject{sub}"),
                                 pl_loggers.CSVLogger(f'./logs/LaBraM/Seed{seed}', name="MoEEG_PhysioP300_csv", version=f"subject{sub}",)])
