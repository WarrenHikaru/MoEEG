...
from Layer.Block import MoEBlock,Block
from Layer.Embedding import ConvLayer,PatchEmbed,RotaryEmbedding
from utils import *
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def fix_init_weight(self):
        ...
        for layer_id, layer in enumerate(self.reconstructor_blocks):
            rescale(layer.time_attn.proj.weight.data, layer_id + 1)
...
class EEGTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.ConvEmbedding = ConvLayer(self.embed_dim)

        self.chan_embed = nn.Embedding(len(CHANNEL_DICT), embed_dim)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # depth=8
        ...
        for i in range(depth):
            if i in [2, 5]:
                ...
                self.blocks.append(
                    Block(
                        dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias,num_patches=self.num_patches[1], drop=drop_rate, attn_drop=attn_drop_rate,
                ...
                self.blocks.append(
                    Block(
                        dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate,
                        drop_path=dpr[i], norm_layer=norm_layer,use_gate=False
                    )
                )
    ...
    def fix_init_weight(self):
        ...
        def init_moe(moe_module, layer_id):
            if hasattr(moe_module, 'gate'):
                nn.init.xavier_uniform_(moe_module.gate.weight)
            ...
            if hasattr(moe_module, 'experts'):
                for expert in moe_module.experts:
                    ...
                    rescale(expert.fc2.weight.data, layer_id)
                    if expert.fc2.bias is not None:
                        nn.init.zeros_(expert.fc2.bias)

            if hasattr(moe_module, 'output_proj'):
                rescale(moe_module.output_proj.weight.data, layer_id)
                if moe_module.output_proj.bias is not None:
                    nn.init.zeros_(moe_module.output_proj.bias)

        for layer_id, layer in enumerate(self.blocks):
            rescale(layer.time_attn.proj.weight.data, layer_id + 1)

            if layer_id != 2 and layer_id != 5:
                rescale(layer.mlp.fc2.weight.data, layer_id + 1)
            else:
                init_moe(layer.moe, layer_id + 1)
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_t=None):
        ...
        x = x.permute(0, 2, 1, 3).contiguous()  # [B,N,C,T1]

        conv_embed = self.ConvEmbedding(x)
        ...
        # Channel Embedding
        x = time_patch + conv_embed + self.chan_embed(chan_ids.long()).unsqueeze(0)  # (1,C) -> (1,1,C,D)
